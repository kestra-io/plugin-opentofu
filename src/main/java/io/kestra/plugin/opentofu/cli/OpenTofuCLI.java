...
public class OpenTofuCLI extends Task implements RunnableTask<ScriptOutput>, NamespaceFilesInterface, InputFilesInterface, OutputFilesInterface {
    private static final String DEFAULT_IMAGE = "ghcr.io/opentofu/opentofu";

    @Schema(
        title = "Pre-run setup commands",
        description = "Commands executed before `commands`, typically `tofu init`."
    )
    ...
    @Schema(
        ...
        description = "Used only when the task runner is container-based; defaults to `ghcr.io/opentofu/opentofu`."
    )
    @Builder.Default
